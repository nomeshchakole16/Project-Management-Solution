...
const AssignProjectToTeammember = () => {
  ...
  useEffect(() => {
    const getAllTeammember = async () => {
      const allTeammember = await retrieveAllTeammembers();
      if (allTeammember) {
        setAllTeammembers(allTeammember.users);
      }
    };
  ...
  const retrieveAllTeammembers = async () => {
    const response = await axios.get(
      "http://localhost:8080/api/user/teammember/all"
    );
    ...
    fetch("http://localhost:8080/api/project/update", {
      ...
      result.json().then((res) => {
        ...
        if (res.success) {
          ...
          setTimeout(() => {
            navigate("/user/teamlead/project/all");
      ...
      <div className="mt-2 d-flex aligns-items-center justify-content-center">
        ...
        >
          <div className="card-header bg-color text-center custom-bg-text">
            <h5 className="card-title">Assign Project To Team Member</h5>
          </div>
          <div className="card-body text-color">
            ...
            <form>
              <div class="col">
                <label htmlFor="quantity" className="form-label">
                  <b>Assign Project To Team Member</b>
                </label>
                <select
                  name="teammemberId"
...
export default AssignProjectToTeammember;

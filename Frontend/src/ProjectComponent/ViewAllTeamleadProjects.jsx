...
const ViewAllTeamleadProjects = () => {
  ...
  const retrieveAllProject = async () => {
    const response = await axios.get(
      "http://localhost:8080/api/project/fetch/teamlead?teamleadId="+teamlead.id
    );
  ...
  const retrieveProjectByName = async () => {
    const response = await axios.get(
      "http://localhost:8080/api/project/teamlead/search?projectName=" + projectName +
        "&teamleadId=" +
        teamlead.id
    );
    console.log(response.data);
  ...
  const assignToTeammember = (project) => {
    navigate("/project/assign/teammember", { state: project });
  };
                ...
                {allProjects.map((project) => {
                  return (
                    <tr>
                      ...
                      <td>
                        {(() => {
                          if (project.assignedToTeammember === "Not Assigned") {
                            return (
                              <button
                                onClick={() => assignToTeammember(project)}
                                className="btn btn-sm bg-color custom-bg-text"
                              >
                                <b>Assign To Team Member</b>
                              </button>
                            );
                          }
                        })()}
...
export default ViewAllTeamleadProjects;
